                                                        ...
                                                        <select
                                                            ...
                                                            @foreach ($macoquan as $item)
                                                                <option value="{{ $item->id }}"
                                                                    {{ isset($vanban) && $vanban->ma_co_quan == $item->id ? 'selected' : '' }}>
                                                        ...
                                                        <div class="col-lg-12 mb-3">
                                                            ...
                                                            @if ($column['type'] === 'text')
                                                                <textarea class="form-control {{ $errors->has($column['name']) ? 'is-invalid' : '' }}" name="{{ $column['name'] }}"
                                                                    id="example-text-input-{{ $column['name'] }}" placeholder="{{ $column['comment'] }}">{{ isset($vanban) ? $vanban->{$column['name']} : old($column['name']) }}</textarea>
                                                            @else
                                                                <input
    ...
    <script>
        $jq(document).ready(function() {
            $jq('#agency_code-select').change(function() {
                ...
                $jq('#hop_so-select').find('option').remove().end().append(
                    '<option value="">Hộp số</option>').val('');
                                ...
                                $jq('#ma-phong-select').change(function() {
                                    ...
                                    $jq('#hop_so-select').find('option').remove().end()
                                        .append('<option value="">Hộp số</option>').val(
                                            '');
                                    $jq('#ho_so_so-select').find('option').remove()
                                        .end()
